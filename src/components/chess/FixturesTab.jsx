...
export const FixturesTab = ({ currentDivision, gameResults }) => {
  ...
  const downloadAsImage = async (round) => {
    const element = round === 'all' 
      ...
      : document.getElementById(`fixture-round-${round}`);
    ...
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-3xl p-6 border border-gray-100 shadow-sm">
        <div className="flex flex-col gap-1">
          <h3 className="font-space text-base font-black text-[#111111]">Round Fixtures</h3>
          <p className="text-xs font-semibold text-gray-400">Download active fixtures as a styled image or formatted PDF</p>
        </div>
      ...
      <div id="fixtures-list" className="space-y-8 bg-transparent">
        {activeRoundsList.map((r) => (
          <div key={r.round} id={`fixture-round-${r.round}`} className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
