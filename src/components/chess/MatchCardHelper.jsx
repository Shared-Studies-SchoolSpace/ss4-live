    ...
    <div className={`flex items-center gap-3 md:gap-4 w-full ${align === 'right' ? 'justify-end' : 'justify-start'} min-w-0`}>
      {align === 'right' && (
        <div className="text-right min-w-0 flex-1">
          <span className="text-brand-text-dark font-bold text-sm md:text-base tracking-wide truncate block group-hover:text-brand-primary transition-colors">
            {name}
          </span>
          <div className="flex items-center justify-end gap-1.5 mt-0.5">
            ...
            <span className="text-[10px] md:text-xs font-semibold text-gray-500 truncate">
              @{username}
            </span>
            <span className={`text-[10px] font-black px-1.5 py-0.5 rounded border ${platformBg} ${platformColor}`}>
              {details.rating}
            </span>
          </div>
        </div>
