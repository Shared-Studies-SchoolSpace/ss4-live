    ...
    <div className={`flex items-center gap-3 md:gap-4 w-full ${align === 'right' ? 'justify-end' : 'justify-start'} min-w-0`}>
      {align === 'right' && (
        <div className="text-right min-w-0 flex-1">
          <span className="text-brand-text-dark font-bold text-xs sm:text-sm md:text-base tracking-wide truncate block group-hover:text-brand-primary group-hover:underline transition-colors cursor-pointer">
            {name}
          </span>
          <div className="flex flex-wrap items-center justify-end gap-1 mt-0.5">
            ...
            <span className="text-[9px] sm:text-[10px] md:text-xs font-semibold text-gray-500 group-hover:text-brand-primary group-hover:underline truncate max-w-[50px] sm:max-w-none cursor-pointer">
              @{username}
            </span>
      ...
      <div className="relative shrink-0">
        {isWinner && (
          <span 
            className="material-symbols-outlined absolute -top-2.5 -right-1.5 text-blue-500 font-black text-[18px] drop-shadow-[0_1.5px_3px_rgba(0,0,0,0.7)] select-none z-10"
            style={{ fontVariationSettings: "'FILL' 1, 'wght' 700" }}
          >
            crown
          </span>
        )}
        <div className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-gray-100 border border-gray-200 rounded-full overflow-hidden flex items-center justify-center shadow-md`}>
          {details.loading ? (
